import { Layers, Lightbulb, Bluetooth, ArrowLeftRight, RefreshCw, Usb, Unplug } from 'lucide-react'
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center justify-between border-b border-black/5 px-5 py-3">
          <div className="flex items-center gap-2">
            {lightConnected ? (
              <Usb size={15} className="text-brand" />
            ) : (
              <Unplug size={15} className="text-faint" />
            )}
            <span className="text-[12px] font-bold">ESP32 實體燈光</span>
          </div>
          <span
            className={`rounded-full px-2 py-1 text-[11px] font-bold ${
              lightConnected ? 'bg-open-soft text-brand' : 'bg-app text-faint'
            }`}
          >
            {lightConnected ? '已連線' : '離線'}
          </span>
        </div>
        ...
        <div className="border-b border-black/5 px-5 py-4">
          <div className="mb-2.5 flex items-center justify-between">
            <span className="text-[13px] font-semibold">燈光亮度</span>
            <span className="font-mono text-[13px] font-medium" style={{ color: mode.color }}>
              {ledBrightness}%
            </span>
          </div>
          <input
            ...
            aria-label="燈光亮度"
            className="h-2 w-full cursor-pointer appearance-none rounded-full disabled:cursor-not-allowed disabled:opacity-40"
            style={{
              background: `linear-gradient(to right, ${mode.color} ${ledBrightness}%, #eae8e1 ${ledBrightness}%)`,
              accentColor: mode.color,
            }}
          />
        </div>
